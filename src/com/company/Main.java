...
public class Main{
    ...
    public static double srSquare( Strana [] cntr){
        double s=0;
        for (int i = 0; i < cntr.length; i++)
            s+=cntr[i].square;
    ...
    public static Strana [] Bigger(Strana cntr[]){
        ...
        if (count != 0){
            Strana [] bigCountry=new Strana[count];
            int n=-1;
            for (int i = 0; i < cntr.length; i++)
                if (cntr[i].square>sred) {
                    bigCountry[++n]=cntr[i];
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Средняя площадь ="+srSquare(country));
